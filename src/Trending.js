...
const App = (props) => {
	...
	const onSaveTrending = () => {
		...
		// check movieDataFromDB if it had any null
		if (Object.keys(movieDataFromDB.streaming_info).length === 0) {
			const OttProvider = values.ott_channel;
			const streamInfo = {
				[OttProvider]: { in: { link: values.ott_url } }
			};
		}

		if (movieDataFromDB.trailer === null || movieDataFromDB.trailer === '') {
			movieDataFromDB.trailer = values.trailer;
		}

		if (movieDataFromDB.runtime === null || movieDataFromDB.runtime === '') {
			movieDataFromDB.runtime = values.runtime;
		}
		...
		axios(SERVER_URL + '/onSaveTrending', {
			method: 'post',
			// headers: {
	...
	const setMovieDataFromTrending = (movieDataX) => {
		console.log(movieDataX);
		setMovieDataFromDB(movieDataX);
		setMovieAlreadyPresentFlag(true);
		console.log(movieDataX);
		const movieData = movieDataX;
				...
				>
					<div style={{ border: '0.5px solid rgba(105,105,105, .3)', height: 500, marginRight: 10 }}>
						<div style={{ position: 'relative', top: -15, left: 15 }}>
							<Typography style={{ fontWeight: '600', fontSize: 20 }}>Movies</Typography>
						</div>
						<MovieList
							setMovieDataFromTrending={(movieData) => setMovieDataFromTrending(movieData)}
							...
							<div style={{ display: 'flex', flexDirection: 'row' }}>
								...
								<div style={{ marginTop: 15 }}>
									<Autocomplete
										...
										renderInput={(params) => (
											<TextField {...params} label="Adult" variant="outlined" />
										)}
									/>
								</div>
							</div>
							...
							>
								...
								<div style={{ marginLeft: 50 }}>
									...
									>
										...
										<FormControlLabel value="friendList" control={<Radio />} label="Friend List" />
									</RadioGroup>
								</div>
								<Button
									style={{
